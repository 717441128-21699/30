...
import { Building2, LogOut, FileBarChart, Bell, User, Menu, X, Shield } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const roleColorClass: Record<UserRole, string> = {
    teller: 'from-cyan-500 to-blue-600 shadow-glow-blue',
    supervisor: 'from-fuchsia-500 to-purple-600',
    operation: 'from-emerald-500 to-teal-600 shadow-glow-green',
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 pl-2 ml-1 border-l border-cyan-800/50">
            ...
            <div className="hidden sm:block">
              ...
              <div className="text-[9px] text-cyan-500 font-orbitron tracking-wider flex items-center gap-1">
      ...
      <div className="flex-1 flex relative overflow-hidden">
        {leftOpen && (
          <aside className="absolute left-0 top-0 bottom-0 w-80 z-10 p-2 space-y-2 overflow-y-auto pointer-events-auto">
            <QueuePanel />
            <ATMPanel />
            {perm.canViewForecast && <ForecastPanel />}
          </aside>
        )}

        <main className="flex-1 relative">
          <Canvas shadows camera={{ position: [10, 10, 12], fov: 50 }} gl={{ antialias: true, alpha: false }}>
            <BankScene />
        ...
        {rightOpen && (
          <aside className="absolute right-0 top-0 bottom-0 w-80 z-10 p-2 space-y-2 overflow-y-auto pointer-events-auto">
            {perm.canTriggerEmergency ? (
              <EmergencyPanel />
            ) : (
              <div className="hud-panel rounded-lg p-3">
                <div className="text-[10px] font-orbitron text-slate-400 text-center py-6">
                  <Shield className="w-6 h-6 mx-auto mb-2 text-slate-500" />
                  您的角色无应急指挥权限
                </div>
              </div>
            )}
            {perm.canViewVIP && <VIPPanel />}
            {perm.canViewWorkOrders && <WorkOrderPanel />}
          </aside>
        )}
      </div>
    </div>
